...
class IntegrationHealthReport {
  const IntegrationHealthReport({required this.version, required this.ok, required this.firebase, required this.llm, required this.langfuse, required this.s3});

  final String version;
  final bool ok;
  final DependencyCheck firebase;
  final DependencyCheck llm;
  final DependencyCheck langfuse;
  ...
  factory IntegrationHealthReport.fromJson(JsonMap json) => IntegrationHealthReport(
    ...
    langfuse: DependencyCheck.fromJson(json['langfuse'] as JsonMap),
    s3: DependencyCheck.fromJson(json['s3'] as JsonMap),
  );

  JsonMap toJson() => {'version': version, 'ok': ok, 'firebase': firebase.toJson(), 'llm': llm.toJson(), 'langfuse': langfuse.toJson(), 's3': s3.toJson()};
}
