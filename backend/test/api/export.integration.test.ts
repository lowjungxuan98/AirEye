...
import { AIREYE_API_KEY, API_KEY_HEADER, buildTestApp, silentLogger } from "../test-utils";
...
describe("GET /api/v1/export (HTTP integration)", () => {
  it("returns 200 with empty data and pagination fields when there are no uploads", async () => {
    ...
    const res = await request(app).get("/api/v1/export").set(API_KEY_HEADER, AIREYE_API_KEY).expect(200);
  ...
  it("caps limit at EXPORT_MAX_LIMIT", async () => {
    ...
    const res = await request(app)
      .get(`/api/v1/export?limit=${EXPORT_MAX_LIMIT + 10}`)
  ...
  it("returns page 2 slice and is_next when more rows exist", async () => {
    ...
    const res = await request(app)
      .get("/api/v1/export?page=2&limit=2")
      .set(API_KEY_HEADER, AIREYE_API_KEY)
      .expect(200);
  ...
  it("sets is_next true on page 1 when a further page exists", async () => {
    ...
    const res = await request(app)
      .get("/api/v1/export?page=1&limit=2")
      .set(API_KEY_HEADER, AIREYE_API_KEY)
      .expect(200);
  ...
  it("returns 400 for a non-numeric limit query", async () => {
    const app = buildTestApp();
    const res = await request(app)
      .get("/api/v1/export?limit=not-a-number")
      .set(API_KEY_HEADER, AIREYE_API_KEY)
      .expect(400);
    expect(res.body).toEqual({
  ...
  it("returns 400 for a non-numeric page query", async () => {
    const app = buildTestApp();
    const res = await request(app)
      .get("/api/v1/export?page=not-a-number")
      .set(API_KEY_HEADER, AIREYE_API_KEY)
      .expect(400);
    expect(res.body).toEqual({
  ...
  it("returns a stable ApiError payload when listUploads rejects with ApiError", async () => {
    ...
    const res = await request(app).get("/api/v1/export").set(API_KEY_HEADER, AIREYE_API_KEY).expect(503);
    expect(res.body).toEqual({
  ...
  it("returns 500 INTERNAL_ERROR and logs for unexpected errors", async () => {
    ...
    const res = await request(app).get("/api/v1/export").set(API_KEY_HEADER, AIREYE_API_KEY).expect(500);
    expect(res.body).toEqual({
